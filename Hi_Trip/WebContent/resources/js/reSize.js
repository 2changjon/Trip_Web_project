var resize_num = 0;
var windowWidth_ch = false;
window.addEventListener('resize', function(event) {
    console.clear();
	var windowWidth = $( window ).width();
	if( windowWidth < 830 && windowWidth > 500) {
		$("#map *").remove();//내부요소 삭제
		...
		$(".basic_time").removeClass("show");//시간 공간
		select_country.style.display = 'none'; //시간
		contents_basic.innerHTML = "Welcome"+'<br>&nbsp;&nbsp;&nbsp;'+"Earth"+'&nbsp;&nbsp;&nbsp;'; //초기화
		$("#country_serch").val(""); //초기화
		country_datas = null; //초기화
		focused = false; //초기화
		var side_show = $("#side").hasClass("show");//사이드바가 켜져있는 채로 변경되었는지
		...
		if( height > width ){
			...
			if(side_show){$("#globeSvg").attr("class", "back");}	
		...
		windowWidth_ch = true;
		//티켓 검색 전에만 reload();
		if(resize_num == 5 && ticket_serch_ck == false){
			location.reload();
		}
		
		resize_num++;
	}else if(windowWidth_ch){
		$("#map *").remove();//내부요소 삭제
